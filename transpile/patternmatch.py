    ...
    return ast.Assign(
        ...
        value=ast.Call(
            func=ast.Attribute(
                ...
                attr=f"convert_{attr.__class__.__name__}",
                ctx=ast.Load(),
            ),
            args=[
                ast.Attribute(
                    value=ast.Name(id="node", ctx=ast.Load()), attr=name, ctx=ast.Load()
                )
            ],
            keywords=[],
        ),
...
NODE__CLASS__NAME__ = ast.Attribute(
    value=ast.Attribute(
        value=ast.Name(id="node", ctx=ast.Load()), attr="__class__", ctx=ast.Load()
    ),
    attr="__name__",
    ctx=ast.Load(),
)
...
class LuaAstMatch(PatternMatching):
    def __init__(self) -> None:
        ...
        self.first_match = ast.Match(subject=NODE__CLASS__NAME__, cases=[])
        self.nodes = []
        self.attribute_keys = []
    ...
    def _add_node_type_match(self, node):
        ...
        self.node_match_cases[key] = ast.match_case(
            pattern=mc_pattern,
            body=[body_part],
        )
    ...
    def new_attribute_sequence(self, name, attr):
        return ast.MatchSequence(
            patterns=[
                ast.MatchValue(value=string(name)),
        ...
        for name, attr in node.__dict__.items():
            ...
            if isinstance(attr, last.Node):
                ...
                if name.startswith("_") == False:
                    k += attr.__class__.__name__
                    match_sequence.patterns.append(
                        self.new_attribute_sequence(name, attr)
                    )
                    key.append(k)
                    actions.append(make_astwriter_method(node, name, attr))
    ...
    def add_attribute_case(self, node: last.Node, attr_key: str, case: ast.match_case):
        try:
            ...
            self.node_attribute_match_cases[node.__class__.__name__] = {attr_key: case}
        self.node_match_cases[node.__class__.__name__].body[0].cases.append(
            self.node_attribute_match_cases[node.__class__.__name__][attr_key]
        )
