...
from transpile.luaparser.parser.LuaLexer import LuaLexer
...
class Builder:
    ...
    def parse_block(self) -> Block:
        ...
        self.handle_hidden_right()
        return Block(
            [self.polymorph(x) for x in statements],
            first_token=t,
    ...
    def parse_tail(self) -> Node | bool:
        ...
        if self.next_is_rc(Tokens.COL) and self.next_is_rc(Tokens.NAME):
            ...
            if self.next_is_rc(Tokens.OPAR):
                ...
                if self.next_is_rc(Tokens.CPAR, False):
                    ...
                    return Invoke(None, self.polymorph(name), expr_list)
        ...
        if self.next_is_rc(Tokens.OPAR, False):
            ...
            if self.next_is_rc(Tokens.CPAR, False):
                self.success()
                # noinspection PyTypeChecker
                return self.polymorph(Call(None, expr_list, last_token=self._LT))
    ...
    def parse_expr(self) -> Expression | bool:
        return self.polymorph(self.parse_or_expr())
    ...
    def polymorph(self, node: Node):
        if isinstance(node, Call) and isinstance(node.func, Name):
            ...
            if node.func.id == "require":
                return Require(func=node.func, args=node.args)
            # METATABLE
